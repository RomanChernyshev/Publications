...
## Схема хранения данных(РЕДАКТИРОВАТЬ)
...
## Поиск(РЕДАКТИРОВАТЬ)
Пришло время задуматься о самом главном. Данные организованы в виде документов. Как мы привыкли осуществлять поиск по документу? 
...
## Распределенность(РЕДАКТИРОВАТЬ, СТРУКТУРИРОВАТЬ)
Быстрой поиск по индексу есть. Но что если поисковые запросы будут приходить так часто, что наш средненький сервер не успеет их обраотать, ведь на такой популярной площадке одновременно могут искать тысячи пользователей и для каждого нужно успеть предложить autocomplete, делать поиск без него просто неприлично. Помимо этого, что будет, если сервер на котором развернут поиск ляжет?

Очевидно - поисковую систему необходимо распределить на несколько машин.

### Распределенный индекс
Ранее Арчибальд предполагал, что все проиндексированные данные будут храниться в истансе Lucene. Однако сейчас индекс должен быть на всех машинах на которых будет развернуто приложение. Каждый инстанс Lucene должен стать частью этого одного больго индекса, или осколком(**shard**) разбитого индекса.
...
**Shard** будет выполнять непосредственно операции по поиску, добавлению и удалению данных.

Но если будет несколько шардов, как понять, в какой именно нужно положить новую запись или, в каком именно производить поиск? Ведь если мы будем рандомно делать этот выбор, очень скоро в нашей "распределенной системе" наступит хаос, данные попросту будут отличаться для каждого нового запроса.

Необходимо добавить немного организованности. Вот если бы запущенные инстансы приложения могли общаться друг с другом, можно было бы организовать их работу.
